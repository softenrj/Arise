...
export default function MiniPlayerTrackDetails() {
    ...
    const panGesture = Gesture.Pan()
        ...
        .onUpdate((event) => {
            ...
            opacity.value = Math.max(0.2, 1 - Math.abs(event.translationX) / SWIPE_OFFSET);
        })
        .onEnd((event) => {
            if (event.translationX < -SWIPE_THRESHOLD) {
                translateX.value = withTiming(-SWIPE_OFFSET, { duration: 100 }, (finished) => {
                    if (finished) runOnJS(skipToNext)();
                });
                opacity.value = withTiming(0, { duration: 100 });
            } else if (event.translationX > SWIPE_THRESHOLD) {
                translateX.value = withTiming(SWIPE_OFFSET, { duration: 100 }, (finished) => {
                    if (finished) runOnJS(skipToPrevious)();
                });
                opacity.value = withTiming(0, { duration: 100 });
            } else {
                resetPositions();
